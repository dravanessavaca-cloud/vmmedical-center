...
interface LoginPageProps { onLogin: (email: string, password: string) => Promise<boolean>; loading: boolean; error: string | null }

export function LoginPage({ onLogin, loading, error }: LoginPageProps) {
  const [showPass, setShowPass] = useState(false)
  const { register, handleSubmit, formState: { errors } } = useForm<LoginForm>()
  const onSubmit = async (data: LoginForm) => { await onLogin(data.email, data.password) }
  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex w-[45%] bg-gradient-to-br from-[#0a2e40] via-[#0d4a35] to-teal-600 flex-col items-center justify-center p-12 relative overflow-hidden">
        ...
        <p className="z-10 absolute bottom-8 text-white/30 text-xs">Sistema Clínico Integral v1.0 · Fase 1</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-gray-50">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 uppercase tracking-wide mb-1.5">Correo electrónico</label>
              <div className="relative">
                <Mail size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="email" placeholder="usuario@vmmedical.ec" autoComplete="email"
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 uppercase tracking-wide mb-1.5">Contraseña</label>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-xs">{showPass ? 'Ocultar' : 'Ver'}</button>
              </div>
            ...
            <Button type="submit" loading={loading} className="w-full py-2.5 mt-2">Ingresar al sistema</Button>
          </form>
          <p className="text-xs text-gray-400 text-center mt-6">VM Medical Center · Sistema Clínico Integral</p>
        </div>
      </div>
    </div>
  )
