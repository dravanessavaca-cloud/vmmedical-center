// ============================================================
// VM Medical Center — Hook de citas médicas
// ============================================================
...
export function useAppointments({ userId, role }: UseAppointmentsOptions) {
  ...
  const fetchAppointments = useCallback(async (filters?: {
    ...
    let query = supabase
      ...
      .select(`
        *,
        patient:patients(id, first_name, last_name, id_number, phone, medical_record_number),
        professional:profiles(id, full_name, specialty, role)
      `)
      .order('appointment_date', { ascending: true })
      .order('appointment_time', { ascending: true })

    // Médico y podólogo solo ven sus propias citas
    if (role === 'medico' || role === 'podologo') {
  ...
  const createAppointment = useCallback(async (input: AppointmentInsert): Promise<Appointment | null> => {
    const { data, error: err } = await supabase
      .from('appointments')
      .insert({ ...input, created_by: userId, status: 'pendiente' })
    ...
    setAppointments(prev => [...prev, appt].sort((a, b) => a.appointment_time.localeCompare(b.appointment_time)))
    return appt
  }, [userId])
  ...
  const updateAppointment = useCallback(async (id: string, updates: AppointmentUpdate): Promise<boolean> => {
    ...
    await logAuditEvent({
      action: 'UPDATE', tableName: 'appointments', recordId: id,
